...
**English (ELA): grading OUT OF SCOPE; corrupted data CLEANED ✅** — The 11 items tagged `type:"written"` across 6 files (aug-2018, jan-2018, jan-2026, jun-2016/17/18) were crawler garbage — mislabeled reading-passage fragments / source attributions (URLs, line-number rulers, "Printed on Recycled Paper"), no `choices`, `modelAnswer:null` — and have been **removed** (pure deletion, 78 lines; each file now holds its intact 24-question Part 1 reading-comprehension set). The real ELA essays (Part 2 argument, Part 3 text-analysis) were never captured by the crawler and remain absent. AI grading stays out of scope for ELA because those essays are scored 0–6 holistically by anchor papers, which doesn't map to the single-`modelAnswer` shape `gradeWriting` needs. To ever add ELA grading: re-crawl/source the two essay prompts + their texts, then build a band-rubric grading path (not this loop).
