...
// Bump when SYSTEM/CONCEPT_SYSTEM or the schemas change, so tutorCache stops
// serving entries generated by the previous prompt/shape. Without this a schema
// change is invisible to every question anyone has already asked about.
const TUTOR_CACHE_VERSION = 'v2'

// The five mistake types the retry loop reasons about. Keep in sync with
// MISTAKE_TYPES in mobile/src/utils/reviewQueue.js.
...
have), produce four escalating coaching levels. The student sees them one at a
time, in this order, and stops as soon as they can finish on their own — so each
level must stand alone and must NOT give away the level below it:
...
- "firstStep": the single opening move on THIS question — the first thing to
  write down or check. Stop there. Do not carry it through to the answer.
...
Also classify the student's error as "mistakeType", exactly one of:
- "concept_gap": they don't yet understand the underlying idea.
- "careless": they knew the method but slipped (sign, arithmetic, transcription).
- "reading_trap": they misread the question or answered a different question
  than the one asked.
- "formula_setup": right concept, wrong formula, setup, or units.
- "test_strategy": a testing error — ran out of patience, guessed, or fell for
  the most familiar-looking choice.
Pick the single best fit. If the wrong choice is equally consistent with several,
prefer the one a teacher would coach first.
...
Using ONLY the supplied stimulus, question, choices, correct answer, and
explanation (plus general subject knowledge a Regents student is expected to
have), produce four escalating levels, revealed to the student one at a time:
...
- "method": 1–2 sentences on how to recognize and approach this kind of question
  next time.
- "firstStep": the opening move on a question like this — where to start. Stop
  there; do not work it through to the answer.
...
const LADDER_REQUIRED = ['nudge', 'method', 'firstStep', 'explanation']

// Mistake mode classifies the error; concept mode has no wrong choice to
// classify, so it gets the ladder alone rather than a fabricated mistakeType.
const SCHEMA = {
  type: 'object',
  additionalProperties: false,
  properties: { ...LADDER_PROPS, mistakeType: { type: 'string', enum: MISTAKE_TYPES } },
  required: [...LADDER_REQUIRED, 'mistakeType'],
}

const CONCEPT_SCHEMA = {
  type: 'object',
  additionalProperties: false,
  properties: { ...LADDER_PROPS },
  required: [...LADDER_REQUIRED],
}

// Suffix distinguishes per-wrong-choice mistake explanations from the single
// shared concept explanation (mode === 'concept' → '__concept'). The version
// prefix retires entries written under an older prompt or schema.
  ...
  async (req) => {
    ...
    try {
      resp = await client.messages.create({
        ...
        // Four ladder rungs + a classification. On the `hard` path adaptive
        // thinking shares this budget with the text, so it needs real headroom.
        max_tokens: hard ? 2000 : 900,
        ...(hard ? { thinking: { type: 'adaptive' } } : {}),
        system: concept ? CONCEPT_SYSTEM : SYSTEM,
        output_config: { format: { type: 'json_schema', schema: concept ? CONCEPT_SCHEMA : SCHEMA } },
