import { UNITS, getByTopic, allQuestions } from '../content/geometry/units'
import { predictRegentsScore } from '../utils/predictedScore'
import { isUnitUnlocked } from '../utils/unitUnlocks'

// Guards the unit-metadata invariants added for the Geometry restructure
// (~/.claude/plans/expressive-meandering-lagoon.md), mirroring
// esUnitModel.test.js and lifeScienceUnitModel.test.js.

describe('geometry UNITS metadata', () => {
  ...
  // Regression guard: the Congruence/Similarity/Circles/Coordinate Geometry/
  // 3D Geometry/Trigonometry families were all dissolved into sub-topic
  // units. Every one of the 382 pooled questions (excluding the geometry-pr
  // skill overlay, which intentionally reuses questions already counted in
  // their own topic unit) should be served by exactly one unit's pool.
  it("every question is served by exactly one non-skill unit's pool", () => {
    const total = allQuestions().length
    const sum = UNITS
      .filter((u) => u.id !== 'geometry-pr')
