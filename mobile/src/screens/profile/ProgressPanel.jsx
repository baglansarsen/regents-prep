...
// The Progress dashboard — the former ProgressScreen body, now a panel inside the
// combined Profile tab. Renders the achievements row via the Profile stack's
// 'Achievements' route (rehomed from the removed Progress stack).
export default function ProgressPanel({ navigation }) {
  const { C } = useTheme()
  ...
  const achHistory = history.filter((h) => {
    ...
    return sub === 'living-environment' || sub === 'earth-science' || sub === 'life-science' || sub === 'geometry' || sub === 'chemistry'
  })
